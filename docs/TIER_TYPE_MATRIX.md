# Tier × Type Matrix: Understanding Memory Organization

## Overview

Memory Indexer uses **two orthogonal dimensions** to organize memories:

1. **Tier** (Storage Layer): WHERE the memory is stored
   → Based on cognitive architecture (Atkinson-Shiffrin, Tulving, Baddeley)

2. **Type** (Memory Classification): WHAT kind of memory it is
   → Based on content classification (Episodic, Semantic, Procedural, Fact)

**Key Insight**: These dimensions are **independent** and **orthogonal**.
→ Any memory Type can exist in any Tier.

## 2D Matrix: Tier × Type Combinations
...
### T0: SensoryBuffer (Atkinson-Shiffrin Sensory Memory)
...
- Automatic promotion to Working when thresholds met
...
### T1: WorkingMemory (Baddeley's Working Memory)
...
### T2: EpisodicStore (Tulving's Episodic Memory)
...
### T3: SemanticStore (Tulving's Semantic Memory)
...
**Tier Distribution**:
- **SensoryBuffer**: Raw episodic conversation turns
- **WorkingMemory**: Recent session context chunks
- **EpisodicStore**: Archived session summaries (primary home)
- **SemanticStore**: Repeatedly referenced past events ("user always mentions...")
...
**Tier Distribution**:
- **SensoryBuffer**: Raw factual statements from conversation
- **WorkingMemory**: Recently discussed facts
- **EpisodicStore**: Facts extracted from sessions
- **SemanticStore**: Confirmed long-term facts (primary home)
...
**Tier Distribution**:
- **SensoryBuffer**: Raw workflow descriptions
- **WorkingMemory**: Currently discussed procedures
- **EpisodicStore**: Archived workflow discussions
- **SemanticStore**: Confirmed standard operating procedures
...
**Tier Distribution**:
- **SensoryBuffer**: Raw factual statements
- **WorkingMemory**: Recently mentioned facts
- **EpisodicStore**: Facts from past sessions
- **SemanticStore**: Confirmed persistent facts (primary home)
...
**SemanticStore (T3)** — Extracted Knowledge:
```
Entry 1: [Semantic]
  Key: "auth_preference"
...
Tier: T2 (EpisodicStore)
Promotion Reason: WorkingMemory TTL expired
Importance: 0.6
```

**Stage 4: Confirmed Knowledge (T3 SemanticStore)**
```
Timestamp: 2024-12-20 14:30:00 (after 3rd mention)
Key: "ui_theme_preference"
Value: "dark_mode"
Type: Semantic
Tier: T3 (SemanticStore)
...
**Stage 2: Working Context (T1 WorkingMemory)**
```
...
**Stage 3: Session Memory (T2 EpisodicStore)**
```
...
**Stage 4: Pattern Extraction (T3 SemanticStore)**
```
...
### ❌ Wrong: "EpisodicStore only contains Episodic type"

**Counterexample**:
```
EpisodicStore can contain:
...
**Why?** EpisodicStore (Tier) stores SESSION MEMORIES. Those sessions can discuss ANY TYPE of content.

---

### ❌ Wrong: "SemanticStore only contains Semantic type"

**Counterexample**:
```
SemanticStore can contain:
...
**Why?** SemanticStore (Tier) stores LONG-TERM CONFIRMED KNOWLEDGE. Any repeatedly confirmed knowledge qualifies, regardless of Type.
...
**Counterexample**:
```
WorkingMemory can hold:
- Recent conversation context (yes, most common)
- Retrieved historical context (recalled from EpisodicStore for current task)
- Long-term facts (fetched from SemanticStore for current topic)
```

**Why?** WorkingMemory (Tier) represents ACTIVE CONTEXT. Retrieval can pull ANY type from ANY tier into Working for immediate use.
...
### ❌ Wrong: "Once in SemanticStore, memories never change tier"

**Correct**: Retrieval can bring SemanticStore entries back to WorkingMemory.

**Example Flow**:
```
1. SemanticStore: "preferred_language: Python" (T3, confirmed)
2. User asks: "What's my preferred language?" (retrieval query)
3. WorkingMemory: Add "User prefers Python" to active context (T3 → T1)
4. Response uses WorkingMemory content
5. After session ends, returns to SemanticStore (T1 → T3)
```
...
### ❌ Wrong: "Procedural memories must be in SemanticStore"

**Counterexample**:
```
Procedural in SensoryBuffer:
  "To deploy: run tests, build, push" (raw instruction)

Procedural in WorkingMemory:
  Currently discussing deployment workflow

Procedural in EpisodicStore:
  Session about deployment procedure discussion

Procedural in SemanticStore:
...
```csharp
// Store Semantic fact - starts in SensoryBuffer (T0)
...
// Tier: T0 (SensoryBuffer) - automatic assignment
...
```csharp
// Access SensoryBuffer directly (T0)
var sensoryBuffer = serviceProvider.GetRequiredService<ISensoryBuffer>();
await sensoryBuffer.EnqueueAsync("Raw conversation text", "user123");

// Access WorkingMemory directly (T1)
var workingMemory = serviceProvider.GetRequiredService<IWorkingMemory>();
var activeContext = workingMemory.GetAll();  // All types in T1

// Access EpisodicStore directly (T2)
var episodicStore = serviceProvider.GetRequiredService<IEpisodicStore>();
var sessionMemories = await episodicStore.GetSessionsAsync("user123");

// Access SemanticStore directly (T3)
var semanticStore = serviceProvider.GetRequiredService<ISemanticStore>();
...
**Promotion Logic**:
- **SensoryBuffer → WorkingMemory**: Time/Token/Turn thresholds (OR)
- **WorkingMemory → EpisodicStore**: Time/Token/Turn/Topic thresholds (OR)
- **EpisodicStore → SemanticStore**: Confidence + Confirmation thresholds (AND)
